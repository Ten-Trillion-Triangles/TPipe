...
- Add `GenericOpenAIEndpointProfile`, including protocol-correct defaults, `localV1()` routes, and explicit MiniMax compatibility, then lock profile changes after the first request.
...
`GenericOpenAIEndpointProfile` owns only validated relative endpoint paths. The default profile returns the standard Anthropic `/v1/messages` route and the existing OpenAI/Responses suffixes; `localV1()` returns `/v1/chat/completions`, `/v1/responses`, and `/v1/messages`. `miniMax()` explicitly retains `/anthropic/v1/messages` for that provider's route convention. Both Ktor and direct streaming request construction already call the shared `getEndpoint()` path, so profile selection stays transport-only.
...
- protocol-correct default profile, explicit `miniMax()` compatibility, `localV1()` values, path validation, first-request locking, non-streaming Ktor URL capture, and direct-streaming URL/header capture;
...
The focused GenericOpenAI, Responses dispatch, and profile command passes all 24 tests. The Defaults module passes all 43 tests, including the factory regression. The full GenericOpenAI module remains red only in five unrelated existing live/token-tracing tests (missing live credentials and pre-existing Anthropic streaming token expectations). `git diff --check` is clean, and the new transport/profile tests make no real model calls.
